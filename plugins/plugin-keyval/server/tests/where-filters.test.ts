...
describe("Where Filters", () => {
  ...
  describe("$eq (equal)", () => {
    it("should filter by exact string match", async () => {
      await seedUsers();
      const results = await collectList<TestUser>(["users"], {
  ...
  describe("$in", () => {
    it("should filter by value in array", async () => {
      ...
      expect(results.length).toBe(4);
      expect(results.every((u) => ["active", "pending"].includes(u.status))).toBe(true);
    });
  ...
  describe("$contains", () => {
    it("should filter strings containing substring", async () => {
      ...
      expect(results.length).toBe(2);
      expect(results.every((u) => u.email?.includes("@example"))).toBe(true);
    });
