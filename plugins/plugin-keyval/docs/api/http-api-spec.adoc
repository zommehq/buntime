...
| string
| "strong"
| Nível de consistência: "strong" ou "eventual"
|===
...
curl -X GET "http://localhost:4000/_/plugin-keyval/keys/users/123?consistency=eventual"
----
...
Remove uma chave ou múltiplas entradas por filtro.
...
IMPORTANT: Se `where` for fornecido, a chave é tratada como prefixo e todas as entradas correspondentes ao filtro são deletadas.
...
----
{
  ...
  "consistency": "strong",       // opcional, nível de consistência
...
curl -N "http://localhost:4000/_/plugin-keyval/watch?keys=users/123,posts/456"
...
| Tamanho de chave
| Sem limite
| Array de partes (string, number, bigint, boolean, Uint8Array)

| Limite de listagem
| 1000
| Máximo de entradas retornadas por vez
