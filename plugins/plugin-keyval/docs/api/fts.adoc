...
await fetch("http://localhost:8000/api/keyval/indexes", {
  method: "POST",
...
[source,http]
----
GET /api/keyval/search?prefix=["posts"]&query=typescript&limit=20
----
...
[source,bash]
----
curl "http://localhost:8000/api/keyval/search?prefix=%5B%22posts%22%5D&query=typescript+tutorial&limit=10"
----
...
const response = await fetch("http://localhost:8000/api/keyval/search", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    prefix: ["posts"],
...
curl http://localhost:8000/api/keyval/indexes

# 2. Criar índice para o prefixo
curl -X POST http://localhost:8000/api/keyval/indexes \
