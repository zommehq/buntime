...
await fetch("http://localhost:8000/_/plugin-keyval/indexes", {
  method: "POST",
...
[source,http]
----
DELETE /_/plugin-keyval/indexes?prefix=["posts"]
----
...
| `prefix`
| `string`
| Prefixo do índice a ser removido (JSON serializado)
|===
...
[source,http]
----
GET /_/plugin-keyval/search?prefix=["posts"]&query=typescript&limit=20
----
...
[source,bash]
----
curl "http://localhost:8000/_/plugin-keyval/search?prefix=%5B%22posts%22%5D&query=typescript+tutorial&limit=10"
----
...
const response = await fetch("http://localhost:8000/_/plugin-keyval/search", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    prefix: ["posts"],
...
curl http://localhost:8000/_/plugin-keyval/indexes

# 2. Criar índice para o prefixo
curl -X POST http://localhost:8000/_/plugin-keyval/indexes \
