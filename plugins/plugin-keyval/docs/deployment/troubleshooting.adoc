...
// 1 query
await kv.get(ids.map(id => ["users", id]));
----
...
console.log('Database URL:', process.env.LIBSQL_URL_0);
...
----
# Runner logs
DEBUG=buntime:* bun run runtime/index.ts
...
curl http://localhost:4000/api/keyval/metrics

# Prometheus format
curl http://localhost:4000/api/keyval/metrics?format=prometheus

# Queue stats
curl http://localhost:4000/api/keyval/queue/stats
----
