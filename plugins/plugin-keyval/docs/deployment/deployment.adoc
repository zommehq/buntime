...
Configure o plugin no `manifest.jsonc`:
...
----
# Health check
curl http://localhost:8000/keyval/api/health
...
│                 │ HTTP │  (/keyval/api)  │ HTTP │  (rede interna) │
└─────────────────┘      └──────────────────────┘      └─────────────────┘
----
...
==== Todas as Opções do manifest.jsonc
