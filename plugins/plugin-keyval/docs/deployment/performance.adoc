...
TIP: Para operações em massa, sempre prefira `getMany()` ou `atomic()` em vez de múltiplas chamadas individuais.
...
TIP: Para ambientes de alta latência, maximize o uso de operações em batch como `getMany()` e `atomic()`.
...
* **Batch reads**: ~50,000 keys/s (getMany com 100 keys)
