...
----
{
  ...
  "libsqlToken": "${LIBSQL_TOKEN}"  // Variável de ambiente
}
----
...
Use sempre o protocolo `libsql://` para conexões criptografadas:

[source,typescript]
----
// CORRETO - TLS habilitado (produção)
"libsqlUrl": "https://libsql.internal:8080"

// HTTP OK apenas em rede interna isolada
"libsqlUrl": "http://libsql:8080"
----

==== Exposição Interna vs Externa

O plugin KeyVal expõe rotas em `/_/plugin-keyval/*`:
...
----
// Válido
await kv.getMany([key1, key2, ..., key1000]);  // 1000 chaves

// Inválido - excede limite
await kv.getMany([key1, key2, ..., key1001]);  // 1001 chaves
----
