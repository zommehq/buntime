...
----
{
  "plugins": [
    ["@buntime/plugin-keyval", {
      ...
      "libsqlReplicaUrl": "${LIBSQL_REPLICA_URL}", // URL da réplica (opcional)
      "libsqlToken": "${LIBSQL_TOKEN}",       // Token de autenticação (opcional)
      "cleanupInterval": 60000,               // Intervalo de limpeza de expirados (ms)
      "operationTimeout": 30000               // Timeout de operações (ms)
    }]
  ]
}
----
