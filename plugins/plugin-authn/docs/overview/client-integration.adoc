...
O tipo `User` inclui todos os campos retornados pelo endpoint de sessão:
...
NOTE: Os campos `groups`, `roles` e `metadata` são armazenados como strings JSON no banco de dados. A API deserializa esses campos automaticamente, retornando arrays e objetos conforme mostrado acima.

==== Identity

O tipo `Identity` representa os dados injetados no header `X-Identity`:
