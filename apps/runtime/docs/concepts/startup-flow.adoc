...
export function initConfig(buntimeConfig: BuntimeConfig, baseDir: string): RuntimeConfig {
  ...
  // 3. Valida LIBSQL_URL (obrigatória)
  const libsqlUrl = options.libsqlUrl ?? Bun.env.LIBSQL_URL;
  if (!libsqlUrl) throw new Error("LIBSQL_URL is required");

  // 4. Define poolSize por ambiente
  const poolSize = poolDefaults[NODE_ENV];  // dev: 10, prod: 500

  return { workerDirs, pluginDirs, libsqlUrl, poolSize, ... };
}
----
...
O banco de dados LibSQL é inicializado no startup:
...
async function initDatabase(): Promise<void> {
  ...
  // Conecta ao servidor LibSQL
  client = createClient({
    url: config.libsqlUrl,      // LIBSQL_URL (obrigatória)
    authToken: config.libsqlAuthToken,  // LIBSQL_AUTH_TOKEN (opcional)
  });
