...
NOTE: Recomendamos usar um único namespace `zomme` para todos os serviços (buntime, libsql, etc). Isso simplifica a comunicação entre serviços (ex: `http://libsql:8080` em vez de `http://libsql.libsql:8080`).
...
. **Certificates**: (via kubectl) `kubectl get certificates -n zomme`
...
[source,bash]
----
# Verificar se libSQL está rodando (mesmo namespace)
...
# Opcional: remover namespace (cuidado: remove todos os serviços!)
...
| Atualizar
| `helm upgrade buntime ./charts/buntime -n zomme -f values-k3s.yaml`
...
| Shell no pod
| `kubectl exec -n zomme -it deployment/buntime -- sh`
|===
