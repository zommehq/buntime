...
function QueuePage() {
  const { t } = useTranslation("keyval.queue");
  ...
  const handleStartListening = useCallback(() => {
    setListening(true);
    listenerRef.current = kv.listenQueue((value) => {
    ...
    <ScrollArea className="h-full">
      <div className="m-4 space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardDescription>{t("stats.dlq")}</CardDescription>
        ...
        <Tabs defaultValue="enqueue">
          ...
          <TabsContent className="space-y-4" value="enqueue">
            <Card>
              <CardHeader>
                <CardTitle>{t("enqueue.title")}</CardTitle>
                <CardDescription>{t("enqueue.description")}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid gap-4">
                  <div className="space-y-2">
                    <Label>{t("enqueue.valueLabel")}</Label>
                    <Input
                      placeholder={t("enqueue.valuePlaceholder")}
                      value={messageValue}
                      onChange={(e) => setMessageValue(e.target.value)}
                    />
                  </div>
                  <div className="space-y-2">
                    <Label>{t("enqueue.delayLabel")}</Label>
                    <Input
                      placeholder={t("enqueue.delayPlaceholder")}
                      type="number"
                      value={delay}
          ...
          <TabsContent className="space-y-4" value="listen">
            <Card>
              <CardHeader>
                <CardTitle>{t("listen.title")}</CardTitle>
                <CardDescription>{t("listen.description")}</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label>
                    {t("listen.received")} ({receivedMessages.length})
                  </Label>
          ...
          <TabsContent className="space-y-4" value="dlq">
            <Card>
              <CardHeader>
                <CardTitle>{t("dlq.title")}</CardTitle>
                <CardDescription>{t("dlq.description")}</CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead>{t("dlq.table.id")}</TableHead>
                      <TableHead>{t("dlq.table.value")}</TableHead>
                      <TableHead>{t("dlq.table.attempts")}</TableHead>
                      <TableHead>{t("dlq.table.error")}</TableHead>
                      <TableHead className="w-[120px]">{t("dlq.table.actions")}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
...
export const Route = createFileRoute("/keyval/queue/")({
  component: QueuePage,
  loader: () => ({ breadcrumb: "keyval:nav.queue" }),
});
