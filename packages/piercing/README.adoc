= Piercing
:toc: left
:toclevels: 3
:sectnums:
:sectnumlevels: 5
:icons: font
:source-highlighter: highlightjs

ifdef::env-github[]
:tip-caption: 💡
:note-caption: ℹ️
:important-caption: ⚠️
:caution-caption: 🔥
:warning-caption: ⚠️
endif::[]

toc::[]

== Introdução

Piercing é um framework client-side para micro-frontends, permitindo compor fragmentos de interface de múltiplas aplicações em uma única página com streaming HTML e comunicação em tempo real.

NOTE: Server-side pre-piercing é fornecido pelo `@buntime/plugin-piercing`. Este pacote contém apenas o código client-side.

=== O que é Piercing?

Piercing permite que você:

* **Componha fragmentos** - Integre múltiplos fragmentos em uma única página usando web components
* **Streaming HTML** - Fragmentos são carregados via streaming usando `writable-dom`
* **Compartilhe estado** - Use MessageBus para comunicação entre fragmentos
* **Isolamento de CSS** - Shadow DOM isola estilos entre shell e fragmentos
* **Isolamento de histórico** - Estratégias `patch` e `isolate` para navegação

=== Uso Básico

[source,html]
----
<!-- No shell HTML -->
<fragment-outlet src="/metrics" base="/cpanel"></fragment-outlet>
<fragment-outlet src="/health" base="/cpanel"></fragment-outlet>
----

[source,typescript]
----
// No entry point do shell
import { registerPiercingComponents } from "@buntime/piercing/client";

registerPiercingComponents();
----

=== Quando usar Piercing?

[cols="1,1"]
|===
| Bom para: | Evitar para:

| Micro-frontends compostos
| Aplicações monolíticas simples

| Composição de equipes independentes
| Projetos com única equipe

| Plugins com UI independente
| Componentes compartilhados via biblioteca

| Isolamento de CSS/JavaScript
| Aplicações com estilo unificado
|===

== Web Components

Os web components são a base do Piercing no cliente.

include::docs/client/web-components.adoc[leveloffset=+1]

include::docs/client/fragment-host.adoc[leveloffset=+1]

== MessageBus

Sistema de comunicação entre fragmentos.

include::docs/message-bus/client-side.adoc[leveloffset=+1]

== Sandbox

Estratégias de isolamento para execução de fragmentos.

include::docs/sandbox/strategies.adoc[leveloffset=+1]

== Pre-Piercing (Server-Side)

Server-side pre-piercing é fornecido pelo plugin `@buntime/plugin-piercing`.

include::docs/gateway/pre-piercing.adoc[leveloffset=+1]

== Exemplos

include::docs/examples/basic-setup.adoc[leveloffset=+1]

== API Reference

=== Web Components

==== fragment-outlet

Placeholder que carrega e renderiza um fragmento.

[cols="1,1,3"]
|===
| Atributo | Tipo | Descrição

| `src`
| string (required)
| URL para buscar o fragmento

| `base`
| string
| Base path do shell para contexto de roteamento

| `history`
| "patch" \| "isolate"
| Estratégia de isolamento de histórico
|===

==== fragment-host

Container para conteúdo do fragmento (usado internamente e no pre-piercing).

[cols="1,1,3"]
|===
| Atributo | Tipo | Descrição

| `src`
| string (required)
| URL do fragmento (mesmo valor do outlet)
|===

=== MessageBus

[source,typescript]
----
import { getBus, dispatch, subscribe } from "@buntime/piercing/client";

// Obter instância do bus
const bus = getBus();

// Dispatch evento
dispatch("user:login", { id: 1, name: "Alice" });

// Subscribe a evento
const unsubscribe = subscribe("user:login", (user) => {
  console.log("User logged in:", user);
});

// Cleanup
unsubscribe();
----

=== Types

[source,typescript]
----
import type {
  JSONValue,
  MessageBus,
  MessageBusCallback,
  MessageBusState,
  NavigationItem,
} from "@buntime/piercing";
----
