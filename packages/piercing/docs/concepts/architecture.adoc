...
O Sandbox permite isolar fragmentos externos (de domínios diferentes) com diferentes níveis de isolamento. Existem quatro estratégias:
...
Interceptação via Service Worker. Permite controle de rede sem iframe.
...
**Quando usar**: Fragmentos externos que precisam interceptar fetch mas compartilhar DOM
...
6. Combina shell + fragment:
   - Fragment envolto em <piercing-fragment-host>
...
7. Response streaming para browser:
   <html>
     ...
     <body>
       ...
       <piercing-fragment-host fragment-id="logs">
...
8. Browser executa:
   ...
   - PiercingFragmentOutlet procura host
...
[source,typescript]
----
class PiercingFragmentOutlet extends HTMLElement {
...
[source,typescript]
----
class PiercingFragmentHost extends HTMLElement {
...
<piercing-fragment-outlet fragment-id="settings" />
----
...
<piercing-fragment-outlet fragment-id="notifications" />
----
...
* **none/monkey-patch/service-worker**: MessageBus compartilhado (mesmo objeto)
