...
Pre-piercing é habilitado através do plugin `@buntime/plugin-piercing`. Para usar, adicione o plugin ao arquivo `manifest.jsonc`:

[source,jsonc]
----
// plugins/plugin-piercing/manifest.jsonc
{
  "enabled": true
}
----
...
[source,console]
----
[plugin-piercing] Failed to fetch fragment from /metrics: 500
----
...
<html>
  <head>
    ...
    <script type="module" src="/piercing/client.js"></script>
  </head>
  <body>
    <nav>...</nav>
    <main>
