...
<body>
  ...
  <main>
    <fragment-outlet src="/p/sidebar" />
    <div class="content">
      <!-- Conteúdo principal -->
    </div>
  </main>

  <fragment-outlet src="/p/footer" />
</body>
----
...
<fragment-outlet src="/p/navigation?theme=dark" />
----
...
function Dashboard() {
  ...
  return (
    <div>
      {user.role === "admin" && (
        <fragment-outlet src="/p/admin-panel" />
      )}
    </div>
  );
}
...
[source,typescript]
----
// Fragment service running on /p/metrics
app.get("/", async (req) => {
  return new Response(
    `
...
<html>
  ...
  <body>
    ...
    <fragment-outlet src="/p/navigation" />
    ...
    <script type="module">
      ...
      if (showMetrics) {
        ...
        outlet.setAttribute("src", "/p/metrics");
  ...
  return new Response(
    ...
    <script type="module">
      ...
      customElements.whenDefined("fragment-host").then(() => {
        const host = document.querySelector("fragment-host[src='/p/navigation']");
  ...
  return new Response(
    ...
    <div class="metrics-dashboard">
      ...
      <script type="module">
        ...
        const evtSource = new EventSource("/p/metrics/stream");
