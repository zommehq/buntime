...
<html lang="pt-BR">
  ...
  <body>
    ...
    <main>
      ...
      <fragment-outlet src="/deployments" base="/dashboard">
...
| `src`
| string (required)
| URL para buscar o fragmento (ex: `/metrics`)
...
1. Servidor retorna HTML shell com `<fragment-outlet src="/metrics">`
2. Browser renderiza shell e registra web components
3. `<fragment-outlet>` faz fetch para `/metrics`
