...
==== Nível 1: Isolamento de Navegação (monkey-patch, service-worker)
...
interface SandboxConfig {
  ...
  /** Origem externa (obrigatória para iframe/service-worker) */
  origin?: string;
...
* **strategy**: Uma de `"none"` | `"monkey-patch"` | `"iframe"` | `"service-worker"`
* **origin**: URL base do fragmento externo (ex: `"https://external.com"`)
  - Obrigatório para `iframe` e `service-worker`
  - Não usado em `none` e `monkey-patch`
...
| service-worker
| Médio
| +100-200ms inicial (registro SW), latência de rede
|===

==== Memória

* **none/monkey-patch**: Compartilham memória da shell
* **iframe**: Contexto separado (~5-10MB adicional por iframe)
* **service-worker**: Cache do SW (~1-2MB)
...
**Preload de Service Worker:**

[source,typescript]
----
// Inicializar SW no startup, antes de precisar
window.addEventListener("load", async () => {
  await initPiercingServiceWorker();
});
----
...
* **Monkey-patch**: Use breakpoints normais, scripts no contexto principal
...
[source]
----
[Piercing] Monkey-patch sandbox activated for: admin-app
[Piercing] Iframe sandbox activated for: external-dashboard
[Piercing] Service-worker sandbox activated for: third-party-widget
----
