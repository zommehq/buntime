...
import { useFiltersState, useJoinOperatorState } from "./data-table-parsers";
import type { ExtendedColumnFilter, FilterOperator, JoinOperator } from "./data-table-types";
import { getDefaultFilterOperator, getFilterOperators } from "./data-table-utils";
...
export function DataTableFilterList<TData>({
  ...
  const [filters, setFilters] = useFiltersState<TData>({
    columnIds,
    key: table.options.meta?.queryKeys?.filters ?? "filters",
  });
  const debouncedSetFilters = useDebouncedCallback(setFilters, debounceMs);

  const [joinOperator, setJoinOperator] = useJoinOperatorState(
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button variant="outline" size="sm" className="font-normal" onKeyDown={onTriggerKeyDown}>
            <Icon icon="lucide:list-filter" className="text-muted-foreground" />
            Filter
            {filters.length > 0 && (
              <Badge
                variant="secondary"
