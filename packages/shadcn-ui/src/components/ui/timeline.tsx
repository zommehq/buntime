...
function TimelineItem(props: DivProps) {
  ...
  const { activeIndex, dir, orientation, variant } = useTimelineContext(ITEM_NAME);
  ...
  const itemContextValue = React.useMemo<TimelineItemContextValue>(
    ...
    [itemId, isAlternateRight, status],
  );
...
function TimelineContent(props: DivProps) {
  ...
  const { orientation, variant } = useTimelineContext(CONTENT_NAME);
  const { isAlternateRight, status } = useTimelineItemContext(CONTENT_NAME);
...
function TimelineDot(props: DivProps) {
  ...
  const { isAlternateRight, status } = useTimelineItemContext(DOT_NAME);
...
function TimelineConnector(props: TimelineConnectorProps) {
  const { asChild, className, forceMount, ...connectorProps } = props;

  const { activeIndex, orientation, variant } = useTimelineContext(CONNECTOR_NAME);
  const { id, isAlternateRight, status } = useTimelineItemContext(CONNECTOR_NAME);
