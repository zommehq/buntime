...
export function AppInstallScreen() {
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      const fileName = resolvedPath.split("/").pop() ?? "app.tgz";
      const file = new File([fileContent], fileName);

      const installResult = await api.installApp(file);
  ...
  return (
    <Layout shortcuts={shortcuts[state] ?? []}>
      <Logo />
      <Label bold>Install App</Label>

      {state === "input" && (
        <Box flexDirection="column" marginTop={1}>
          <TextInput
            focused
            label="Enter path to app (tarball, zip, or directory):"
            onChange={setFilePath}
            onSubmit={handleSubmit}
            placeholder="./my-app.tgz"
            value={filePath}
          />
