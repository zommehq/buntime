...
export const description = "Install a plugin from a tarball (.tgz)";

export const args = z.tuple([z.string().describe("Path to the plugin tarball (.tgz)")]);
...
export default function PluginInstall({ args, options }: Props) {
  const [file] = args;
  ...
  useEffect(() => {
    async function upload() {
      try {
        ...
        const res = await fetch(`${options.url}/api/core/plugins/upload`, {
          body: form,
          headers,
          method: "POST",
        });
  ...
  return (
    <Box flexDirection="column" gap={1}>
      <Text bold>Installing plugin: {file}</Text>
      ...
      {status === "done" && result?.data && (
        <Box flexDirection="column">
          ...
          <Box marginLeft={2} flexDirection="column">
            <Text>
              Name: <Text bold>{result.data.plugin.name}</Text>
            </Text>
            <Text>
              Version: <Text bold>{result.data.plugin.version}</Text>
            </Text>
            <Text>
              Path: <Text color="gray">{result.data.plugin.installedAt}</Text>
            </Text>
          </Box>
        </Box>
      )}
