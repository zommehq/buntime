...
Se você vem de bancos relacionais (PostgreSQL, MySQL) ou NoSQL (MongoDB), este guia vai ajudar a traduzir seus conhecimentos para o modelo KeyVal.

=== O que é KeyVal?

KeyVal é um banco de dados chave-valor hierárquico onde:

* **Chaves** são arrays ordenados: `["users", 123, "profile"]`
* **Valores** são JSON: objetos, arrays, strings, números
* **Operações** são atômicas e consistentes
...
Padrões para estruturar seus dados.
