...
| ID
| Prefixo descritivo + UUID curto
| `usr_abc123`, `pst_xyz789`
...
==== Por que Prefixos em IDs?

[source,typescript]
----
// Sem prefixo: impossível saber o que é
"abc123xyz"  // Usuário? Post? Produto?

// Com prefixo: auto-documentado
"usr_abc123"  // Claramente um usuário
"pst_xyz789"  // Claramente um post
"ord_def456"  // Claramente um pedido

// Facilita debugging em logs
console.log(`Processando ${id}`);
// "Processando usr_abc123" vs "Processando abc123xyz"
----
...
["organizations", orgId, "members", oderId]                 → { role }
...
["stats", "users", oderId, "posts_count"]      → bigint
...
async function createProduct(data: Omit<Product, "id" | "createdAt">): Promise<Product> {
  ...
  const product: Product = { ...data, id, createdAt: Date.now() };
...
async function getProductStats(category?: string): Promise<ProductStats> {
  ...
  const [countEntry, totalEntry, maxEntry] = await kv.getMany([
    [...prefix, "count"],
    [...prefix, "total_value"],
    [...prefix, "max_price"],
  ]);
...
async function recordSale(productId: string, amount: number): Promise<Sale> {
  ...
  const sale: Sale = { id, productId, amount, createdAt: Date.now() };
...
async function getMonthlySalesReport(year: string, month: string) {
  const [countEntry, totalEntry] = await kv.getMany([
...
async function createSession(oderId: string, deviceInfo: string): Promise<string> {
  const sessionId = crypto.randomUUID();

  await kv.set(
    ["sessions", sessionId],
    { userId: oderId, deviceInfo, createdAt: Date.now() },
...
async function isFeatureEnabled(
  ...
  if (flag.value.enabledFor && oderId) {
    return flag.value.enabledFor.includes(oderId);
  }

  // Rollout por porcentagem (baseado no hash do userId)
  if (flag.value.percentage && oderId) {
    const hash = simpleHash(oderId);
...
async function softDeleteUser(oderId: string, deletedBy: string): Promise<void> {
  const result = await kv.transaction(async (tx) => {
    const entry = await tx.get<User>(["users", oderId]);
    if (!entry.value) throw new Error("Usuário não encontrado");
...
// Restaurar
async function restoreUser(oderId: string): Promise<void> {
  const result = await kv.transaction(async (tx) => {
    const entry = await tx.get<User>(["users", oderId]);
    if (!entry.value) throw new Error("Usuário não encontrado");
    ...
    tx.set(["users", oderId], restored);
    tx.set(["users_by_email", restored.email], oderId);
    tx.delete(["deleted_users", oderId]);
  });
...
["org", orgId, "dept", deptId, "team", teamId, "member", oderId, "perm", permId]

// Melhor: entidades separadas com referências
["permissions", permId] → { userId, teamId, orgId, ... }
["users", oderId, "permissions"] → [permId1, permId2]
----
