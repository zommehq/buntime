...
["users", oderId, "posts", postId] → { title, content }

// Estratégia 2: Referência (posts separados)
["users", oderId] → { name, email }
["posts", postId] → { title, content, authorId: oderId }
----
...
const user = await kv.get(["users", oderId]);
...
["users", oderId, "profile"] → { bio, avatar, socialLinks }

// Leituras separadas
const user = await kv.get(["users", oderId]);
const profile = await kv.get(["users", oderId, "profile"]);
...
["users", oderId, "posts", postId] → { title, content, createdAt }
----
...
const posts: Post[] = [];
for await (const entry of kv.list<Post>(["users", oderId, "posts"])) {
...
await kv.delete(["users", oderId]);
// Tudo que começa com ["users", oderId] é removido:
// - ["users", oderId]
// - ["users", oderId, "posts", "post_1"]
// - ["users", oderId, "posts", "post_2"]
// - ["users", oderId, "profile"]
// - etc.
----
...
["posts_by_author", oderId, postId] → postId
----
...
for await (const entry of kv.list(["posts_by_author", oderId])) {
  postIds.push(entry.value);
}

// Passo 2: Buscar posts (batch para eficiência)
const posts = await kv.getMany(postIds.map(id => ["posts", id]));
----
...
// Tabela de junção
["post_tags", postId, tagName] → true
["tag_posts", tagName, postId] → true  // Índice invertido
----
...
async function createPost(post: Post): Promise<string> {
  ...
  for (const tag of post.tags) {
    atomic
      .set(["post_tags", postId, tag], true)
      .set(["tag_posts", tag, postId], true)
...
const authors = await kv.getMany<User>(
...
// Total: 1 query (list) + 1 query (getMany) = 2 queries
----
...
async function updateUserName(oderId: string, newName: string): Promise<void> {
  // Atualiza o usuário
  const user = await kv.get<User>(["users", oderId]);
  if (!user.value) return;

  await kv.set(["users", oderId], { ...user.value, name: newName });
...
// Índice de posts por autor
["users", oderId, "post_refs", postId] → true
----
