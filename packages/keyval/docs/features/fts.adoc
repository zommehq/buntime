...
==== searchPaginated: Busca com Paginação

[source,typescript]
----
kv.searchPaginated<T>(
  prefix: KvKey,
  query: string,
  options?: ListOptions & { cursor?: string }
): Promise<KvPage<T>>
----
...
----
// Primeira página
const page1 = await kv.searchPaginated<Post>(["posts"], "react hooks", {
  limit: 20
});

console.log(page1.entries);   // Primeiros 20 resultados
console.log(page1.cursor);    // Cursor para próxima página
console.log(page1.hasMore);   // true se há mais resultados

// Segunda página
if (page1.hasMore) {
  const page2 = await kv.searchPaginated<Post>(["posts"], "react hooks", {
    limit: 20,
    cursor: page1.cursor
  });
}
...
==== Todas as Opções de list()

A busca FTS suporta todas as opções de `list()`:

[source,typescript]
----
interface ListOptions {
  limit?: number;                    // Máximo de resultados
  reverse?: boolean;                 // Ordem inversa
  consistency?: "strong" | "eventual"; // Modo de leitura
...
O FTS5 suporta operadores avançados para queries complexas.
...
[source,typescript]
----
// Busca: "typescript" AND "tutorial"
kv.search(["articles"], "typescript tutorial")
// Retorna: artigos que contêm ambos os termos
----
...
[source,typescript]
----
// Busca: "typescript" OR "javascript"
...
[source,typescript]
----
// Busca: frase exata "react hooks"
kv.search(["articles"], '"react hooks"')
// Retorna: artigos com a sequência exata "react hooks"
----

==== Proximidade (NEAR)

Busca termos próximos um do outro:

[source,typescript]
----
// Busca: "typescript" e "tutorial" separados por no máximo 5 palavras
kv.search(["articles"], "NEAR(typescript tutorial, 5)")
// Retorna: artigos onde os termos estão próximos
----
...
[source,typescript]
----
// Busca: palavras começando com "react"
kv.search(["articles"], "react*")
// Retorna: artigos com "react", "reactive", "reactjs", etc.
----
...
----
// Busca complexa
kv.search(["articles"], '"typescript tutorial" OR (react* NOT beginner)')
// Retorna: artigos com frase exata "typescript tutorial" OU
//          (qualquer palavra começando com "react" mas sem "beginner")
----

=== Removendo Índices

==== dropIndex: Remover Índice FTS

[source,typescript]
----
await kv.dropIndex(prefix: KvKey): Promise<void>
----
...
async function searchPosts(query: string, page = 0, limit = 10) {
  ...
  const results = await kv.searchPaginated<BlogPost>(
    ["blog", "posts"],
    query,
    {
      where: { status: { $eq: "published" } },
      limit,
      // Usar cursor em vez de offset para melhor performance
    }
  );

  return {
    posts: results.entries.map(e => e.value),
...
// - Tempo: ~50-200ms
const results = await kv.searchPaginated(["articles"], "typescript tutorial", {
  limit: 20
});
----
...
| Flexibilidade
| Alta (operadores, frases, proximity)
...
// Autocomplete usando prefixo
async function autocomplete(query: string, limit = 5) {
  const suggestions: string[] = [];

  // Usar prefixo * para buscar palavras começando com query
  for await (const entry of kv.search<Product>(
    ["products"],
    `${query}*`,
    { limit }
  )) {
    suggestions.push(entry.value.name);
  }
...
// Exemplo de uso
const suggestions = await autocomplete("note"); // ["Notebook", "Notepad", ...]
----
