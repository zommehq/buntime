...
async function tryBecomeLeader(): Promise<boolean> {
  const result = await kv.atomic()
    ...
    .set(LEADER_KEY, { instanceId, timestamp: Date.now() }, {
      expireIn: LEADER_TTL
    })
    .commit();
...
async function maintainLeadership(): Promise<boolean> {
  ...
  // Renovar o TTL
  await kv.set(LEADER_KEY, { instanceId, timestamp: Date.now() }, {
    expireIn: LEADER_TTL
  });
...
function createResilientWatcher(
  ...
  function start() {
    ...
    try {
      ...
      console.error("Falha ao iniciar watch:", error);
...
// Uso
const resilientWatcher = createResilientWatcher(
  ["users", userId],
  (entries) => console.log("Update:", entries)
);
----
