...
Use a opção `expireIn` ao criar um dado:

[source,typescript]
----
import { Keyval } from "@buntime/keyval";

const kv = new Keyval({ libsqlUrl: "..." });

// Expira em 1 hora
await kv.set(["temp", "data"], { value: 123 }, { expireIn: "1h" });

// Expira em 30 minutos
await kv.set(["cache", "result"], data, { expireIn: "30m" });

// Expira em 7 dias
await kv.set(["token", userId], token, { expireIn: "7d" });
----

==== Sintaxe do `expireIn`

O `expireIn` aceita strings no formato:
...
Você também pode combinar unidades:

[source,typescript]
----
// 1 dia e 12 horas
await kv.set(key, value, { expireIn: "1d 12h" });

// 2 horas e 30 minutos
await kv.set(key, value, { expireIn: "2h 30m" });

// 1 hora, 15 minutos e 30 segundos
await kv.set(key, value, { expireIn: "1h 15m 30s" });
----

TIP: Use unidades maiores quando possível: `"1h"` é mais legível que `"3600s"`.
...
await kv.set(["temp"], "value", { expireIn: "5s" });
...
async function createSession(userId: string, data: SessionData) {
  ...
  await kv.set(["session", sessionId], data, { expireIn: "7d" });
...
async function getUserProfile(userId: string) {
  ...
  await kv.set(cacheKey, profile, { expireIn: "1h" });
...
async function checkRateLimit(userId: string): Promise<boolean> {
  ...
  await kv.set(key, count + 1, { expireIn: "1h" });
...
async function extendSession(sessionId: string) {
  ...
  // Re-salvar com novo TTL
  await kv.set(key, result.value, { expireIn: "7d" });
}
----
...
async function createVerificationCode(email: string) {
  ...
  await kv.set(
    ["verify", email],
    { code, expiresAt: kv.now() + 15 * 60 * 1000 },
    { expireIn: "15m" }
  );
...
async function verifyCode(email: string, code: string) {
  ...
  if (kv.now() > result.value.expiresAt) {
...
await kv.set(["cache", userId], data, { expireIn: "1h" });
----

WARNING: Se um dado deve ser temporário, SEMPRE defina `expireIn`. Dados sem TTL ficam no banco para sempre.
...
await kv.set(["cache", "expensive-query"], result, { expireIn: "1s" });

// TTL adequado ao padrão de acesso
await kv.set(["cache", "expensive-query"], result, { expireIn: "5m" });
----
...
await kv.set(["verify", email], code, { expireIn: "7d" });

// Códigos de verificação devem expirar rápido
await kv.set(["verify", email], code, { expireIn: "15m" });
----
...
if (cached.value) {
  await kv.set(cacheKey, cached.value, { expireIn: "1h" });
  return cached.value;
}
