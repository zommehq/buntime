...
*Controle de fluxo:* Tarefas pesadas não bloqueiam requisições HTTP. O usuário recebe uma resposta imediata enquanto o trabalho acontece em background.
...
| Upload retorna rápido, processamento em background
...
| Gera relatório em background, notifica quando pronto
...
[source,typescript]
----
// Inicia um worker que processa mensagens
...
async function agendarPara(payload: unknown, dataHora: Date) {
  const delay = dataHora.getTime() - Date.now();
