...
const handle = kv.listenQueue(async (msg) => {
  try {
    ...
    const resultado = await processarMensagem(msg);
...
kv.listenQueue(async (msg) => {
  ...
  const podeProcessar = await verificarRegras(msg.payload);
  ...
  await executarProcessamento(msg.payload);
  await kv.ackMessage(msg.id);
}, { autoAck: false });
----
