...
kv.listenQueue(async (msg) => {
  console.log("Processando:", msg.value);
  console.log("ID da mensagem:", msg.id);
  console.log("Tentativas:", msg.attempts);

  // Lógica de processamento baseada no tipo
  if (msg.value.type === "send_email") {
    await enviarEmail(msg.value.to, msg.value.subject, msg.value.template);
  }
...
await kv.enqueue(
  ...
  {
    // Tenta novamente com intervalos crescentes (usando strings)
    backoffSchedule: ["1s", "5s", "30s", "2m"]
  }
);
...
O delay pode ser especificado em milissegundos (number) ou como string formatada:
...
// Usando formato de string (mais legível)
await kv.enqueue(
  {
    type: "send_reminder",
    userId: 123,
...
// Outros exemplos de delay com string
await kv.enqueue(data, { delay: "5s" });   // 5 segundos
await kv.enqueue(data, { delay: "30m" });  // 30 minutos
await kv.enqueue(data, { delay: "1h" });   // 1 hora
await kv.enqueue(data, { delay: "7d" });   // 7 dias
----
...
kv.listenQueue(async (msg) => {
  try {
    ...
    await processarTrabalho(msg.value);
...
==== Controle Manual da Fila

Para casos avançados onde você precisa de controle granular sobre o acknowledgment de mensagens, é possível desabilitar o ACK/NACK automático e gerenciar manualmente:

[source,typescript]
----
const handle = kv.listenQueue(async (msg) => {
  try {
    // Processamento pode ter lógica condicional
    const resultado = await processarMensagem(msg.value);

    if (resultado.sucesso) {
      // Confirma processamento bem-sucedido
      await kv.ackMessage(msg.id);
    } else {
      // Rejeita e reenfileira para retry
      await kv.nackMessage(msg.id);
    }
  } catch (error) {
    // Em caso de erro, também reenfileira
    await kv.nackMessage(msg.id);
  }
}, { autoAck: false }); // Desabilita ACK automático
----

*Quando usar controle manual:*

- Validação condicional: Aceitar apenas se certos critérios forem atendidos
- Processamento parcial: ACK mesmo com falhas recuperáveis em partes não críticas
- Logging avançado: Registrar exatamente quando cada mensagem foi confirmada
- Integração com sistemas externos: ACK apenas após confirmação externa
...
await kv.ackMessage(messageId: string): Promise<void>

// Rejeita processamento (reenfileira com backoff)
await kv.nackMessage(messageId: string): Promise<void>
----
...
kv.listenQueue(async (msg) => {
  // Valida formato
  if (!validarFormato(msg.value)) {
    console.warn("Formato inválido, descartando:", msg.id);
    await kv.ackMessage(msg.id); // ACK para descartar
    return;
  }

  // Valida regras de negócio
  const podeProcessar = await verificarRegras(msg.value);

  if (!podeProcessar) {
    console.log("Condições não atendidas, tentando novamente:", msg.id);
    await kv.nackMessage(msg.id); // NACK para retry
    return;
  }

  // Processa normalmente
  await executarProcessamento(msg.value);
  await kv.ackMessage(msg.id);
}, { autoAck: false });
----

WARNING: Quando usar `autoAck: false`, você DEVE chamar `ackMessage()` ou `nackMessage()` explicitamente. Mensagens não confirmadas ficarão presas no estado "processing" e não serão reprocessadas.

==== Configurando Backoff Schedule

Você controla os intervalos entre tentativas usando milissegundos (number[]) ou strings formatadas (string[]):
...
// Outros exemplos de backoff
await kv.enqueue(data, {
  backoffSchedule: ["1s", "5s", "30s"]  // Crescente exponencial
});

await kv.enqueue(data, {
  backoffSchedule: ["10s", "1m", "5m", "30m"]  // Para APIs lentas
});
----
...
const deadMessages = await kv.dlq.list({ limit: 10 });
...
for (const msg of deadMessages) {
  console.log({
    ...
    createdAt: new Date(msg.originalCreatedAt),
    failedAt: new Date(msg.failedAt)
  });
}

// Paginação com limit e offset
const firstPage = await kv.dlq.list({ limit: 10, offset: 0 });   // Primeiras 10
const secondPage = await kv.dlq.list({ limit: 10, offset: 10 });  // Próximas 10
const thirdPage = await kv.dlq.list({ limit: 10, offset: 20 });  // Próximas 10

// Exemplo de paginação dinâmica
async function listDLQPaginated(page: number, perPage: number) {
  const offset = page * perPage;
  return await kv.dlq.list({ limit: perPage, offset });
}

const firstPage = await listDLQPaginated(0, 20); // Página 0: itens 0-19
const secondPage = await listDLQPaginated(1, 20);    // Página 1: itens 20-39

// Ver mensagem específica
const message = await kv.dlq.get("message-id-123");
...
kv.listenQueue(async (msg) => {
  try {
    ...
    await withTimeout(processarJob(msg.value), 30000);
  } catch (error) {
...
kv.listenQueue<JobPayload>(async (msg) => {
  const job = msg.value;
...
kv.listenQueue(async (msg) => {
  console.log("Worker 1 processando:", msg.value.type);
  await processar(msg.value);
});

// Em worker-2 (processo/container 2)
kv.listenQueue(async (msg) => {
  console.log("Worker 2 processando:", msg.value.type);
  await processar(msg.value);
});

// Em worker-3 (processo/container 3)
kv.listenQueue(async (msg) => {
  console.log("Worker 3 processando:", msg.value.type);
  await processar(msg.value);
});
...
async function processar() {
  ...
  using listener = kv.listenQueue(async (msg) => {
    await processarMensagem(msg.value);
  });
