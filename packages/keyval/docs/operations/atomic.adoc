...
await kv.atomic()
  .max(["games", oderId, "highscore"], BigInt(newScore))
  .commit();
...
await kv.atomic()
  .min(["races", oderId, "bestTime"], BigInt(raceTime))
  .commit();
----
...
async function createUser(user: NewUser): Promise<string> {
  ...
  const result = await kv.atomic()
    ...
    .set(["users", id], { ...user, id, createdAt: Date.now() })
    .set(["users_by_email", user.email], id)
...
async function updateEmail(userId: string, newEmail: string): Promise<void> {
  ...
  const result = await kv.atomic()
    ...
    .set(["users", oderId], { ...user, email: newEmail })
...
async function transfer(fromId: string, toId: string, amount: bigint): Promise<void> {
  ...
  const [from, to] = await kv.getMany<Account>([
    ["accounts", fromId],
    ["accounts", toId],
  ]);
...
Às vezes você precisa usar o versionstamp do commit como parte de uma chave:

[source,typescript]
----
const postId = crypto.randomUUID();
const vs = kv.commitVersionstamp();  // Placeholder

await kv.atomic()
  .set(["posts", postId], { title: "Hello" })
  .set(["posts_by_date", vs, postId], postId)  // vs será substituído
  .commit();

// Resultado: ["posts_by_date", "00000000000000010042", "post_123"]
----

Isso garante que a ordenação por versionstamp reflete a ordem exata de criação.
