...
O método `get()` busca uma única entrada por sua chave:
...
=== getMany: Lendo Múltiplos Valores

Para buscar várias chaves, use `getMany()` em vez de múltiplos `get()`:
...
const entries = await kv.getMany<User>([
  ["users", "usr_001"],
  ["users", "usr_002"],
  ["users", "usr_003"],
]);
...
await kv.set(["cache", "item"], data, {
  expireIn: 60000  // Expira em 60 segundos (TTL)
});
----
...
await kv.delete(["logs"], {
  where: {
    $and: [
      { level: { $eq: "debug" } },
      { timestamp: { $lt: kv.now() - 86400000 } }  // Mais de 24h
    ]
  }
});
----
...
// Vantagens: Fácil identificar o tipo em logs/debug
// Desvantagens: Mais longo
----
