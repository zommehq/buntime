...
await kv.transaction(async (tx) => {
  ...
  // Leituras múltiplas
  const [account, settings] = await tx.getMany<Account | Settings>([
    ["accounts", id],
    ["settings", id],
  ]);
});
...
async function transfer(
  ...
  const result = await kv.transaction(async (tx) => {
    // Ler ambas as contas
    const [from, to] = await tx.getMany<Account>([
      ["accounts", fromId],
      ["accounts", toId],
    ]);
...
// Uso: rate limiting
await incrementWithLimit(["rate_limit", oderId, minute], 100);
----
...
[source,typescript]
----
async function toggleLike(postId: string, oderId: string): Promise<boolean> {
  const result = await kv.transaction(async (tx) => {
    const likeKey: KvKey = ["likes", postId, oderId];
    ...
    if (like.value) {
      ...
      tx.set(likeKey, { createdAt: Date.now() });
      tx.sum(["posts", postId, "likeCount"], 1n);
      return true;
    }
  });
...
const result = await kv.transaction(async (tx) => {
  ...
  tx.set(["users", id], { ...user.value, lastLogin: Date.now() });
});
...
await kv.transaction(async (tx) => {
  const [a, b, c] = await tx.getMany([keyA, keyB, keyC]);
  // ... lógica de negócio
  tx.set(keyA, newA);
  tx.set(keyB, newB);
});
