...
Para iterar sobre todos os registros, use `list()` que já é um AsyncIterator:

[source,typescript]
----
// list() já itera automaticamente por todas as páginas
for await (const entry of kv.list<User>(["users"])) {
  console.log(entry.value.name);
}

// Com limite por batch interno (otimização)
for await (const entry of kv.list<User>(["users"], { limit: 100 })) {
  await processUser(entry.value);
}
----

NOTE: `list()` é a forma idiomática de iterar todos os registros. Use `paginate()` apenas quando precisar expor o cursor para o cliente (APIs REST, infinite scroll).
...
// React component com infinite scroll
function UserList() {
  ...
  async function loadMore() {
    ...
    const { users: newUsers, nextCursor } = await res.json();
...
// Usar get com array de chaves (uma requisição)
const entries = await kv.get(userIds.map(id => ["users", id]));
...
// Busca com limite
for await (const entry of kv.search<Article>(["articles"], "react hooks", {
  limit: 20
})) {
  console.log(entry.value.title);
}
...
- Operadores FTS5 avançados (OR, NOT, NEAR, frases exatas) não são suportados - apenas busca simples por palavras
====
