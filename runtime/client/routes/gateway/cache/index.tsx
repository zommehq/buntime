...
function CacheManagement() {
  ...
  const handleInvalidateByKey = async () => {
    ...
    try {
      const response = await api.gateway.cache.invalidate.$post({ json: { key: key.trim() } });
      const data = await response.json();

      if ("error" in data) {
        setError(data.error);
      } else {
        setResult(data as InvalidateResponse);
        setKey("");
      }
    } catch (err) {
  ...
  const handleInvalidateByPattern = async () => {
    ...
    try {
      const response = await api.gateway.cache.invalidate.$post({
        json: { pattern: pattern.trim() },
      });
      const data = await response.json();
  ...
  const handleClearAll = async () => {
    ...
    try {
      const response = await api.gateway.cache.invalidate.$post({ json: {} });
      const data = await response.json();
            ...
            <CardContent>
              <p className="text-sm text-muted-foreground">
                {result.invalidated === "all"
                  ...
                  : t("result.countInvalidated", { count: result.invalidated ?? 0 })}
              </p>
            </CardContent>
          </Card>
        )}
