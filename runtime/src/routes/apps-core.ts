/**
 * Core Apps API Routes (/api/core/apps)
 *
 ...
 */
export function createAppsCoreRoutes() {
  return new Hono()
    .get("/", async (ctx) => {
    ...
    .delete("/:scope/:name/:version", async (ctx) => {
      const { workerDirs } = getConfig();
      const scope = ctx.req.param("scope");
      const name = ctx.req.param("name");
...
export type AppsCoreRoutesType = ReturnType<typeof createAppsCoreRoutes>;
