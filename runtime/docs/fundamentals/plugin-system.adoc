...
interface PluginContext {
  ...
  globalConfig: GlobalPluginConfig;       // buntime.jsonc (appsDir, poolSize)
...
interface FragmentOptions {
  type: "monkey-patch" | "iframe" | "service-worker";
  origin?: string;              // Obrigatorio para iframe/service-worker
...
*Service Worker (Shared Styles)*:

Intercepta requests, injeta sandbox script. Para apps externos que precisam compartilhar estilos.

[source,typescript]
----
fragment: {
  type: "service-worker",
  origin: "https://legacy-app.company.com",
  preloadStyles: "body { opacity: 0; transition: opacity 0.2s; }",
}
----
