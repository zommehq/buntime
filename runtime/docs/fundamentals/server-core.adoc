...
| `Headers`
| Headers HTTP customizados (x-base, x-fragment-route, x-buntime-internal, x-not-found, x-request-id, sec-fetch-dest, sec-fetch-mode)
...
interface RuntimeConfig {
  ...
  configDir: string;       // Diretório de configuração (default: ./data)
...
const getAppDir = createAppResolver(runtimeConfig.workerDirs);
...
const workers = createWorkerRoutes({ config: runtimeConfig, getAppDir, pool, registry });

// 7. Cria app
const app = createApp({ appsCore, pluginsCore, pluginsInfo, workers, ... });
...
NOTE: O modo app-shell é determinado automaticamente quando o homepage app contém um `fragment-outlet`.
