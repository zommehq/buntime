...
│             │ 3. Inicializa config (appsDir, poolSize)
│             │ 4. Cria WorkerPool
...
interface RuntimeConfig {
  appsDirs: string[];      // Diretórios de apps (obrigatório)
  poolSize: number;        // Tamanho do pool de workers
  port: number;            // Porta do servidor
  shell?: string;          // Shell para npm install
...
// 3. Usa config
console.log(runtimeConfig.appsDirs);  // ["/path/to/apps"]
...
NOTE: `BuntimeConfig` (em buntime.jsonc) usa `appsDir` (array), mas `RuntimeConfig` (resultado de `initConfig()`) usa `appsDirs` (plural) internamente.

===== Resolução de Caminhos

Caminhos relativos em `appsDir` são resolvidos contra o diretório do `buntime.jsonc`:

[source]
----
/project/
├── buntime.jsonc         # appsDir: ["./apps", "/abs/path"]
└── apps/
    └── todos/
...
----
{
  "appsDir": ["${HOME}/apps", "${WORKSPACE}/examples"]
}
----
...
// 7. Cria rotas
const deployments = createDeploymentRoutes({ appsDirs, registry });
...
* Configure `appsDir` como array para suportar múltiplos diretórios

[source,typescript]
----
// buntime.jsonc
{
  "appsDir": ["./apps", "./examples", "${HOME}/external-apps"]
}
----
...
[source]
----
Error: appsDir is required: set in buntime.jsonc or APPS_DIR env var
----

Solução: Configure `appsDir` no `buntime.jsonc`:
