...
O servidor Buntime expõe quatro grupos principais de rotas: deployments, plugins info, fragments e worker routes.
...
Gerencia o carregamento dinâmico de fragmentos de plugins para o web component `fragment-outlet`.

==== ALL /fragment/:fragmentId
...
- Não usa workspaces (usa diretório do plugin)
...
3. **Fragment API** - `/fragment/:fragmentId/*` (rotas montadas)
4. **Plugins Info API** - `/api/plugins` (rota montada)
5. **Plugin Apps** - Apps registrados por plugins (ex: `/api/keyval/*`) (catch-all handler)
6. **Worker Routes** - Apps nos workspaces (ex: `/my-app/*`) (catch-all handler)
7. **Homepage/Fallback** - Raiz `/` (catch-all handler)
...
NOTE: Rotas relacionadas a plugins (como Fragment API e Plugin Apps) podem usar formato simplificado `{ "error": "message" }` sem o campo `code`.
