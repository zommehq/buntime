...
| Default
| **Required** - No default, must be set in buntime.jsonc or WORKSPACES_DIR env var
...
TIP: You can also use the WORKSPACES_DIR environment variable with comma-separated paths: `WORKSPACES_DIR=/dir1,/dir2,../dir3`
...
Optional application to serve when no route matches. This app typically orchestrates micro-frontends via fragments.
...
When set, requests that don't match any application route will be forwarded to this application. The homepage app can use piercing fragments to embed other plugins' UIs.
