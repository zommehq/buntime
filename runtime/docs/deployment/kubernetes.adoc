...
| ConfigMap (env)
| Variáveis de ambiente (PORT, POOL_SIZE, etc)

| ConfigMap (config)
| buntime.jsonc com configuração dos plugins
...
* **Config Mount**: buntime.jsonc montado em `/app/buntime.jsonc`
...
[source,bash]
----
# Visualizar ConfigMap de ambiente
kubectl get configmap buntime -o yaml

# Visualizar ConfigMap do buntime.jsonc
...
Em seguida, configure o Buntime:
...
Habilite o plugin de metrics e colete com Prometheus:
