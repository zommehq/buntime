...
This document describes the micro-frontend architecture for Buntime plugins, inspired by Cloudflare's Fragment Piercing strategy:
...
- **Seamless Integration**: Fragments "pierce" into the main dashboard shell
...
│                                                             │
│  ┌───────────────────────────────────────────────────────┐  │
│  │              PiercingGateway (Hono Middleware)        │  │
│  │                                                       │  │
│  │  • Intercepts HTML requests                           │  │
│  │  • Routes /piercing-fragment/:id to plugin workers    │  │
...
interface BuntimePlugin {
  ...
  fragment?: {
    ...
    /** Styles to apply before piercing (for seamless transitions) */
...
=== PiercingGateway for Bun

Adapted from Cloudflare's implementation for Bun's runtime.

[source,typescript]
----
// packages/piercing/src/piercing-gateway.ts
...
export class PiercingGateway {
  ...
  middleware(): MiddlewareHandler {
    return async (ctx, next) => {
      ...
      // 3. For HTML requests, inject pre-pierced fragments
...
[source,typescript]
----
// packages/piercing/src/piercing-fragment-host.ts
...
export class PiercingFragmentOutlet extends HTMLElement {
  ...
  private async fetchFragment(id: string): Promise<ReadableStream> {
    const res = await fetch(`/piercing-fragment/${id}`, {
      headers: {
...
[source,typescript]
----
// packages/piercing/src/message-bus.ts
...
1. **Phase 1**: Create `@buntime/piercing` package with core components
