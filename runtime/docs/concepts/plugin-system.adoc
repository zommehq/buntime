...
{
  "name": "@buntime/plugin-keyval",  // Identificador único (obrigatório)
  "enabled": true,                    // Habilita/desabilita o plugin
  "base": "/keyval",                  // Base path para rotas (obrigatório)
  "entrypoint": "dist/client/index.html",  // Entrypoint do cliente (se houver)
  "dependencies": ["@buntime/plugin-database"],  // Dependências obrigatórias
  "optionalDependencies": [],         // Dependências opcionais
  "fragment": { "type": "patch" },    // Configuração de fragment
...
| string[]
| []
| Plugins opcionais (carregados se disponíveis)

| `fragment`
| object
| -
| Configuração de fragment UI (`{ type: "patch" \| "iframe" }`)
...
Lista completa de plugins builtin: authn, authz, database, deployments, durable, gateway, health, keyval, logs, metrics, piercing, proxy, vhosts.
...
[source,jsonc]
----
// plugins/external-app/manifest.jsonc
{
  "name": "@company/external-app",
  "base": "/external",
  "fragment": {
    "type": "iframe",
