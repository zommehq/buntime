...
Variáveis sensíveis são automaticamente bloqueadas ao passar `env` para workers via `manifest.jsonc`. Os patterns filtrados:
...
Workers são configurados via `manifest.jsonc` no diretório da aplicação.
...
[source,jsonc]
----
// apps/todos-kv/v1/manifest.jsonc
{
  "entrypoint": "index.ts",
  "timeout": "30s",
  "ttl": "5m",
