...
{
  "plugins": [
    "@buntime/plugin-keyval",     // depende de database
    "@buntime/plugin-authn",      // sem dependências
    "@buntime/plugin-database"    // sem dependências
  ]
}
----

A ordem no arquivo de configuração não importa. O runtime reordena automaticamente:
