...
├── buntime.jsonc         # Configuracao do runtime
├── bunfig.toml           # Configuracao Bun (plugins)
├── components.json       # shadcn/ui config
├── client/               # Admin Dashboard (React)
│   ├── index.html        # HTML com <base href>
...
└── server/               # Worker Pool Server (Hono)
    ├── index.ts          # Bun.serve entry
    ├── api.ts            # Hono app (routes aggregator)
    ...
    ├── plugins/
    │   ├── builtin.ts    # Built-in plugin resolvers
    │   ├── loader.ts     # Plugin loader
    │   └── registry.ts   # Plugin registry
    ├── routes/
    │   ├── deployments.ts  # /api/deployments/* routes
    │   ├── plugins-info.ts # /api/plugins route
    │   └── worker.ts       # App routes
    └── utils/
        ├── get-app-dir.ts
        ├── get-entrypoint.ts
        ├── get-public-routes.ts
        ├── glob-to-regex.ts
        └── serve-static.ts
----
...
----
{
  "workspaces": ["./apps"],      // Diretorios das aplicacoes
...
* `GET /api/deployments/*` - Gerenciamento de deployments
* `GET /api/plugins` - Lista plugins carregados

=== Plugin (`/api/{name}/*`)

* `/api/metrics/prometheus` - Metricas Prometheus
* `/api/authn/*` - Endpoints de autenticacao
* `/api/authz/*` - Endpoints de autorizacao
* `/api/keyval/*` - API KeyVal
* `/api/gateway/*` - Cache e rate limit
...
include::docs/fundamentals/client-architecture.adoc[leveloffset=+1]
